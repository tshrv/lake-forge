...
```
      ...
      ┌────────────────┼────────────────┐
      │                │                │
   dbt Artifacts   OpenLineage     Metrics & Logs
```

We'll build this incrementally, just like the data pipeline.

- Inspect and understand dbt artifacts (manifest.json, catalog.json, run_results.json) and learn what information they expose.
- Add OpenLineage to dbt so every model execution emits lineage events.
- Add OpenLineage to Spark so ingestion events join the same lineage graph.
- Deploy Marquez to visualize the complete end-to-end flow.

By the end, you'll be able to click on customer_revenue in Marquez and trace it all the way back to the original TPCH data generated by Spark. That's a compelling demonstration of observability in a modern lakehouse.

manifest.json
Think of it as "The compiled blueprint of my project." Contains
- all models
- refs
- sources
- dependencies
...
catalog.json
Contains metadata collected from the warehouse.
Example:
- customer_revenue
- Columns
- customer_key
...
run_results.json
Probably the most useful. Contains
- Model
- Execution time
- Status
- Error
...
Spark, dbt both emit events (start complete fail etc), pipe to marquez via openlineage, complete observability
...
instead of `dbt run` / `dbt test`:
dbt-ol run
dbt-ol test
dbt-ol build
```

`dbt-ol test` also emits your dbt-expectations results as data-quality facets, so assertion pass/fail appears on the dataset pages in Marquez.